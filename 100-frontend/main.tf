...
resource "aws_autoscaling_group" "frontend" {
  ...
  desired_capacity          = 2   //starting og the autoscaling group
...
# if instance are not healthy within 15min , autoscaling will delete that instance 
  timeouts {
    delete = "15m"
  }
...
resource "aws_lb_listener_rule" "static" {
  ...
  condition {
    host_header {
      values = ["expense-${var.environment}.${var.zone_name}"]  //expense-dev.sujithyerra.online
    }
}
}